...
const Skills = () => {
  ...
  return (
    <section id="skills" className="py-16 md:py-24 bg-transparent dark:bg-[#09090b] relative overflow-hidden font-sans transition-colors duration-300">
        ...
        <div className="relative select-none touch-pan-y"> {/* Prevents text selection while dragging */}
          ...
          <div ref={ref} className="keen-slider !overflow-visible">
            {SkillsInfo.map((category, index) => (
              <div key={index} className="keen-slider__slide h-auto"> {/* h-auto ensures flex stretching */}
                ...
                <div className="group relative h-full bg-white dark:bg-zinc-900/50 rounded-3xl md:rounded-[2.5rem] border border-gray-200 dark:border-white/5 p-6 md:p-8 transition-all duration-500 hover:border-teal-500/30 dark:hover:border-white/10 overflow-hidden flex flex-col shadow-sm dark:shadow-none">
                  ...
                  <div className="flex flex-col items-center mb-6 md:mb-10">
                    <h3 className="text-xl md:text-2xl font-bold text-gray-800 dark:text-white mb-2">{category.title}</h3>
                    ...
                    {category.skills.map((skill) => (
                      <div
                        ...
                        className="relative flex items-center gap-2 md:gap-3 pl-2 pr-3 md:pl-2 md:pr-4 py-2 md:py-2.5 
                                   bg-gray-100 dark:bg-black/40 rounded-full border border-gray-200 dark:border-white/5 
                                   hover:bg-white dark:hover:bg-white/10 hover:border-teal-500/30 dark:hover:border-white/20 active:scale-95
                                   transition-all duration-300 cursor-default"
                      >
                        {/* Circle background for Logo */}
                        <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-white dark:bg-zinc-800 flex items-center justify-center p-1 md:p-1.5 shadow-sm shrink-0">
                          <img
                            src={skill.logo}
                            alt={skill.name}
                        ...
                        <span className="text-xs md:text-sm font-medium text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white transition-colors whitespace-nowrap">
                          {skill.name}
                        </span>
                      </div>
                    ))}
          ...
          <div className="flex justify-center gap-4 mt-8 md:mt-12">
            <button
              ...
              className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 text-gray-800 dark:text-white flex items-center justify-center hover:bg-gray-100 dark:hover:bg-zinc-800 hover:border-gray-300 dark:hover:border-zinc-700 transition-all active:scale-90 shadow-md dark:shadow-none"
